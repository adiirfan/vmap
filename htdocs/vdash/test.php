...
if (empty($_POST['name'])){
	$_SESSION['pesan']   = 'License ID is Empty';
	header("Location:/vdash/license/add");
...
if(mysql_num_rows($result1)<1 && $username = $data->license[0]->app_code && $input = $data->license_info[0]->license_key){
	$sql = "INSERT INTO license (username,licensekey,expired,max,app,status,id)
				VALUES 
			    ('".$data->license[0]->app_code."','".$encryptedkey."','".$data->license[0]->expired_date."','".$data->license[0]->sum_max_pc."','".$data->license[0]->app_code."','1','1')";
