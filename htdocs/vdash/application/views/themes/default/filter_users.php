	...
	<div class="panel-group filter-list" id="filter-list" role="tablist" aria-multiselectable="true">
		...
		if ( $has_filters ) {
			foreach ( $filters as $filter ) {
				$filter_id = $filter['user_filter_id'];
				$filter_name = htmlentities($filter['user_filter_name']);
				$filter_domain = trim($filter['user_filter_domain']);
				$filter_username = trim($filter['user_filter_username']);
				$filter_ad_group = trim($filter['user_filter_ad_group']);
				$filter_action = trim($filter['user_filter_action']);
				$negate = ($filter['user_filter_negate'] ? true : false);
				$on_success = $filter['user_filter_on_success'];
				$on_fail = $filter['user_filter_on_fail'];
					...
					<div class="detail detail-inline">
						<div class="detail-row">
							<div class="detail-label tips"
								...
								<i class="fa fa-question-circle"></i> <?php echo lang('filter_user_name_pattern'); ?>
							</div>
							<div class="detail-info">
								<span class="filter-pattern"><?php
								if ( is_empty($filter_username) ) {
									echo 'n/a';
								} else {
									echo htmlentities($filter_username);
								}
								?></span>
								<button type="button" class="btn btn-primary btn-sm filter-pattern-modify"
									...
									data-filter-pattern-name="username"
									data-filter-pattern="<?php echo htmlentities($filter_username); ?>"><?php echo lang('modify'); ?></button>
							</div>
						...
						<div class="detail-row">
							<div class="detail-label tips"
								data-toggle="tooltip"
								data-container="body"
								title="<?php echo lang('txt_filter_user_domain'); ?>">
								<i class="fa fa-question-circle"></i> <?php echo lang('filter_user_domain'); ?>
							</div>
							<div class="detail-info">
								<span class="filter-pattern"><?php
								if ( is_empty($filter_domain) ) {
									echo 'n/a';
								} else {
									echo htmlentities($filter_domain);
								}
								?></span>
								<button type="button" class="btn btn-primary btn-sm filter-pattern-modify"
									data-filter-id="<?php echo $filter_id; ?>"
									data-filter-name="<?php echo htmlentities($filter_name); ?>"
									data-filter-pattern-name="domain"
									data-filter-pattern="<?php echo htmlentities($filter_domain); ?>"><?php echo lang('modify'); ?></button>
							</div>
						</div>
						<div class="detail-row">
							<div class="detail-label">
								<?php echo lang('filter_user_ad_group'); ?>
							</div>
							<div class="detail-info">
								<span class="filter-pattern"><?php
								if ( is_empty($filter_ad_group) ) {
									echo 'n/a';
								} else {
									echo htmlentities($filter_ad_group);
								}
								?></span>
								<button type="button" class="btn btn-primary btn-sm filter-pattern-modify"
									data-filter-id="<?php echo $filter_id; ?>"
									data-filter-name="<?php echo htmlentities($filter_name); ?>"
									data-filter-pattern-name="ad_group"
									data-filter-pattern="<?php echo htmlentities($filter_ad_group); ?>"><?php echo lang('modify'); ?></button>
							</div>
						</div>
						<div class="detail-row">
							<div class="detail-label tips"
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body flexiwidth">
        <span class="hidden txt-user-name"><?php echo lang('txt_filter_user_name_change_pattern'); ?></span>
        <span class="hidden txt-user-domain"><?php echo lang('txt_filter_user_domain_change_pattern'); ?></span>
        <span class="hidden txt-user-ad-group"><?php echo lang('txt_filter_user_ad_group_change_pattern'); ?></span>
        <br />
      ...
      <div class="modal-body">
      	<div class="form-inline form-equal-label flexiwidth">
      	  ...
      	  <div class="form-group">
      	    <div class="input-group">
      	      <span class="input-group-addon">
      	        <?php echo lang('user_ad_group'); ?>
      	      </span>
      	      <input type="text" class="form-control" size="35"
      	      	id="txt-test-ad-group" />
      	    </div>
      	  </div>
      	</div>
...
<div class="modal fade" id="modal-new-filter" tabindex="-1" role="dialog" aria-labelledby="createFilter" aria-hidden="true" data-backdrop="static" data-show=false">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body flexiwidth">
        <form name="new_filter_form" id="new-filter-form" action="<?php echo site_url('filter/a_user_new_filter'); ?>" method="post" class="form-horizontal flexiwidth">
        	...
        	<div class="form-group">
        		<label for="filter-user-name" class="col-md-4 control-label">
        			<?php echo lang('filter_user_name_pattern'); ?>
        		</label>
        		<div class="col-md-8">
        			<input ttype="text" name="filter_user_name" id="filter-user-name"
        				value="" class="form-control" size="35" placeholder="" />
        		</div>
        	</div>
        	
        	<div class="form-group">
        		<label for="filter-domain" class="col-md-4 control-label">
        			<?php echo lang('filter_user_domain'); ?>
        		</label>
        		<div class="col-md-8">
        			<input ttype="text" name="filter_user_domain" id="filter-domain"
        				value="" class="form-control" size="15" placeholder="" />
        		</div>
        	</div>
        	
        	<div class="form-group">
        		<label for="filter-user-ad-group" class="col-md-4 control-label">
        			<?php echo lang('filter_user_ad_group'); ?>
        		</label>
        		<div class="col-md-8">
        			<input ttype="text" name="filter_user_ad_group" id="filter-user-ad-group"
        				value="" class="form-control" size="15" placeholder="" />
        		</div>
        	</div>
        	...
        	<div class="form-group">
        		...
        		<div class="col-md-8">
        			<input type="checkbox" name="filter_action" id="filter-success" value="allow"
        				data-toggle="toggle"
        				data-width="80"
        				data-on="<?php echo lang('filter_action_allow'); ?>"
        				data-off="<?php echo lang('filter_action_block'); ?>"
...
<div class="modal fade" id="modal-simulate" tabindex="-1" role="dialog" aria-labelledby="simulateFilter" aria-hidden="true" data-backdrop="static" data-show=false" data-business-id="<?php echo $business_id; ?>">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p>
          <?php echo lang('txt_filter_user_simulation'); ?>
        </p>
        
        <div class="form-horizontal">
	        <div class="form-group">
				<label for="sim-username" class="col-md-4 control-label">
					<?php echo lang('user_name'); ?>
				</label>
				<div class="col-md-8">
					<input type="text" id="sim-username"
						value="" class="form-control" size="35" placeholder="" />
				</div>
			</div>
				
			<div class="form-group">
				<label for="sim-domain" class="col-md-4 control-label">
					<?php echo lang('user_domain'); ?>
				</label>
				<div class="col-md-8">
					<input type="text" id="sim-domain"
						value="" class="form-control" size="15" placeholder="" />
				</div>
			</div>
			
			<div class="form-group">
				<label for="sim-ad-group" class="col-md-4 control-label">
					<?php echo lang('user_ad_group'); ?>
				</label>
				<div class="col-md-8">
					<input type="text" id="sim-ad-group"
						value="" class="form-control" size="15" placeholder="" />
				</div>
			</div>
		</div>
...
<div class="modal fade" id="modal-process" tabindex="-1" role="dialog" aria-labelledby="processFilter" aria-hidden="true" data-backdrop="static" data-show=false">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body flexiwidth">
      	<p>
      		<?php echo lang('txt_filter_user_process'); ?>
      	</p>
